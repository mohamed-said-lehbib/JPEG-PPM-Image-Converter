...
// void transf_pgm_col(uint32_t*** t, const char* nom, uint16_t largeur, uint16_t hauteur) {
...
//     for (uint32_t i =0;i<hauteur;i++){
//         for (uint32_t j = 0; j < largeur; j++) {  
//             // if( i*8 +j >= largeur){break;}
//             printf("\nin\n");
//             if (t[(i+7)/8][(j+7)/8]->data[i%8][j%8] > 255){
//                 b = 255;
//             }
//             else {
//                 b= t[(i+7)/8][(j+7)/8]->data[i%8][j%8];
//             }
//             fwrite(&b, sizeof(uint8_t), 1, fichier);
//         }
//     }
