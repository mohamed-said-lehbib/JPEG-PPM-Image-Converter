...
uint8_t ***sur_ech_horiz(uint8_t ***com, infos_comp **infos_img){
    
    //-----------recuperation des donnees------------------//

    uint8_t **y = com[0];           //  recuperation des composantes
    uint8_t **cb = com[1];
    uint8_t **cr = com[2];
    ...
    uint8_t** new_cb_block = malloc(vcb*sizeof(uint8_t*));
    uint8_t** new_cr_block = malloc(vcr*sizeof(uint8_t*));
    for (uint8_t i = 0; i < vcb; i++) {
        new_cb_block[i] = malloc(hy * sizeof(uint8_t));
        new_cr_block[i] = malloc(hy * sizeof(uint8_t));
    }
...
uint8_t ***sur_ech_ver(uint8_t ***com, infos_comp **infos_img){
    ...
    uint8_t** new_cb_block = malloc(vy*sizeof(uint8_t*));
    uint8_t** new_cr_block = malloc(vy*sizeof(uint8_t*));
    for (uint8_t i = 0; i < vy; i++) {
        new_cb_block[i] = malloc(hcb * sizeof(uint8_t));
        new_cr_block[i] = malloc(hcr * sizeof(uint8_t));
    }
...
uint8_t ***sur_ech(uint8_t ***com, infos_comp **infos_img){
    ...
    //--------Cas de sous echantollage horizontalle et verticale-------------//
        ...
        uint8_t*** new_com = sur_ech_horiz(com, infos_img);
